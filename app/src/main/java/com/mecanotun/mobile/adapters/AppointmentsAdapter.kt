...
class AppointmentsAdapter(private var appointments: List<AppointmentDto>) :
    ...
    override fun onBindViewHolder(holder: AppointmentViewHolder, position: Int) {
        ...
        val serviceNames = appointment.services?.joinToString(", ") { service: com.mecanotun.mobile.api.ServiceDto -> service.name } ?: "No Service"
        holder.tvServiceName.text = serviceNames


        val date = appointment.timeslot?.startDay ?: "N/A"
        val time = "${appointment.timeslot?.startTimeFormat ?: ""}-${appointment.timeslot?.endTimeFormat ?: ""}"
